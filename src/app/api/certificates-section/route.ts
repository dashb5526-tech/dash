...
const certificatesSectionFilePath = path.join(process.cwd(), 'src/lib/certificates-section.json');
...
async function getCertificatesSectionData() {
    try {
        ...
        console.error("Error reading certificates section file, returning empty structure.", error);
        return {
